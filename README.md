...
This is the artifact for paper #125 "LOUD: Synthesizing Strongest and Weakest Specifications". 
...
```
mkdir result
docker run -it -v "$(pwd)/result:/aspire/result" kangheep/aspire_oopsla25
```

This will use the directory `result` to store figures and summary text generated from the container.
...
Then run the command `make jar` in the project directory `aspire` to generate a jar file.

Now you can run the tool by the provided script `aspire`, e.g.
```
./aspire examples/toy/ex1-under.sp examples/toy/ex1.sk --synth-under
```
